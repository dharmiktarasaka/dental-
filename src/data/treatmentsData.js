export const treatmentsData = [
  {
    id: "general-dentistry-ahmedabad",
    title: "General Dentistry",
    icon: "Stethoscope",
    shortDescription: "Comprehensive check-ups, teeth cleanings, and fillings for optimal everyday oral health.",
    longDescription: "Our General Dentistry services form the foundation of a lifetime of healthy smiles. We focus on preventive care, early diagnosis, and conservative treatment of common dental issues. Routine exams allow us to catch dental problems before they escalate into painful, expensive emergencies. Combining advanced diagnostic technologies with a gentle, patient-centered touch, we provide teeth cleanings, cavity fillings, and oral cancer screenings in a clean and welcoming environment.",
    image: "https://images.unsplash.com/photo-1629909613654-28e377c37b09?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹500 - ₹2,500",
    benefits: [
      "Prevents severe tooth decay and gum disease",
      "Removes stubborn plaque and tartar buildup",
      "Maintains fresh breath and clean teeth",
      "Early detection of serious oral health issues"
    ],
    procedure: [
      "Visual Examination & Digital X-Rays if needed",
      "Professional Scaling & Polishing to remove plaque",
      "Cavity checks and restoration using tooth-colored composite materials",
      "Personalized preventive counseling and scheduling next regular check-up"
    ],
    faqs: [
      {
        q: "How often should I visit a dentist for a general check-up?",
        a: "We recommend visiting our Ahmedabad clinic at least once every 6 months for a routine check-up and professional teeth cleaning to prevent plaque accumulation."
      },
      {
        q: "Are tooth-colored composite fillings safe?",
        a: "Yes, composite fillings are highly durable, aesthetic, biocompatible, and completely metal-free (amalgam-free)."
      }
    ]
  },
  {
    id: "dental-implants-ahmedabad",
    title: "Dental Implants",
    icon: "ShieldAlert", // Custom or replaced with standard
    shortDescription: "Permanent, natural-looking replacement for missing teeth using premium titanium implants.",
    longDescription: "Dental implants are the gold standard for replacing missing teeth. An implant acts as a prosthetic root made of medical-grade titanium, which fuses naturally with your jawbone in a process called osseointegration. This creates an extremely secure foundation for a custom-crafted porcelain crown. Unlike traditional bridges or dentures, dental implants look, feel, and function exactly like your natural teeth, preventing bone loss and preserving your facial structure.",
    image: "https://images.unsplash.com/photo-1606811971618-4486d14f3f99?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹25,000 - ₹50,000 per implant",
    benefits: [
      "Restores 100% biting and chewing capacity",
      "Prevents jawbone shrinkage and facial sagging",
      "Doesn't require altering adjacent healthy teeth",
      "Can last a lifetime with proper oral hygiene"
    ],
    procedure: [
      "Initial 3D CBCT digital scan and bone density evaluation",
      "Precision surgical placement of the titanium implant under local anesthesia",
      "Healing period (3-4 months) for the bone to fuse with the implant",
      "Placement of a custom crown matching the exact shade of your teeth"
    ],
    faqs: [
      {
        q: "Are dental implants painful?",
        a: "The procedure is performed under local anesthesia, so you will feel no pain during surgery. Mild post-operative soreness is managed with simple medications."
      },
      {
        q: "How long do dental implants last?",
        a: "With excellent oral hygiene and routine check-ups at our Ahmedabad center, implants can last 25 years to a lifetime."
      }
    ]
  },
  {
    id: "root-canal-treatment-ahmedabad",
    title: "Root Canal Treatment",
    icon: "Activity",
    shortDescription: "Save infected, painful teeth and restore functionality with single-visit painless root canal therapy.",
    longDescription: "Root Canal Treatment (RCT) is a highly effective procedure designed to save a severely decayed, infected, or damaged tooth rather than extracting it. When the inner pulp of a tooth becomes inflamed or infected due to deep decay or trauma, it causes severe throbbing pain. Our clinic specializes in advanced, single-visit root canals using digital rotary systems and dental microscopes, making the process faster, highly precise, and completely pain-free.",
    image: "https://images.unsplash.com/photo-1588776814546-1ffcf47267a5?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹4,000 - ₹8,500",
    benefits: [
      "Provides immediate relief from severe dental pain",
      "Saves the natural tooth and maintains bite alignment",
      "Prevents infection from spreading to surrounding jaw tissues",
      "Efficient and comfortable single-visit procedures available"
    ],
    procedure: [
      "Numbing the tooth and surrounding area completely with localized anesthesia",
      "Creating a tiny access opening to remove infected and dead pulp tissue",
      "Thorough cleaning, shaping, and disinfecting of the root canals",
      "Filling and sealing the canals with biocompatible materials, followed by a dental crown"
    ],
    faqs: [
      {
        q: "Is root canal treatment painful?",
        a: "No. With modern anesthetics and rotary technology, root canal treatment is as comfortable as getting a standard cavity filling."
      },
      {
        q: "Why do I need a crown after a root canal?",
        a: "A tooth becomes brittle after its blood supply is removed during root canal therapy. A crown protects it from cracking under chewing pressure."
      }
    ]
  },
  {
    id: "teeth-whitening-ahmedabad",
    title: "Teeth Whitening",
    icon: "Sparkles",
    shortDescription: "Safely brighten your teeth by up to 8 shades in just 45 minutes using advanced laser bleaching.",
    longDescription: "A bright, white smile projects health, youth, and confidence. Over time, consumption of tea, coffee, turmeric, and smoking can stain enamel. Our professional in-office teeth whitening system utilizes safe, light-activated bleaching agents to break down stubborn, deep-set stains. Unlike over-the-counter kits that can damage gums or cause severe sensitivity, our treatment is fully customized, highly supervised, and delivers instant, dazzling results.",
    image: "https://images.unsplash.com/photo-1579684389782-64d84b5e905d?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹7,000 - ₹15,000",
    benefits: [
      "Instant, visible results in under an hour",
      "Safe, clinically tested materials that protect gum health",
      "Effectively removes coffee, tobacco, and turmeric stains",
      "Custom shade matching for a natural, bright appearance"
    ],
    procedure: [
      "Polishing teeth to remove superficial plaque",
      "Applying a protective barrier to shield your gums",
      "Applying the medical-grade whitening gel to enamel",
      "Activating the gel with a specialized blue LED light for three 15-minute cycles"
    ],
    faqs: [
      {
        q: "Is teeth whitening safe for my enamel?",
        a: "Yes, professional teeth whitening is completely safe. The formulation we use contains desensitizing agents that protect enamel and prevent permanent sensitivity."
      },
      {
        q: "How long do the whitening results last?",
        a: "Results typically last 1 to 2 years, depending on your lifestyle and dietary habits (e.g., intake of coffee, tea, and red wine)."
      }
    ]
  },
  {
    id: "smile-designing-ahmedabad",
    title: "Smile Designing",
    icon: "Sparkles",
    shortDescription: "Customized aesthetic smile correction combining digital mockups, veneers, and alignment.",
    longDescription: "Smile Designing, or Digital Smile Design (DSD), is a highly personalized cosmetic procedure that blends science and art to create your dream smile. By analyzing your facial structure, lip symmetry, gum line, and teeth size through digital photography and 3D modeling, we create a custom blueprint of your new smile. We combine porcelain veneers, composite bonding, and gum contouring to resolve issues like chips, gaps, and severe staining, allowing you to preview your smile before treatment begins.",
    image: "https://images.unsplash.com/photo-1513412583791-03a82b057f21?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹30,000 - ₹1,50,000 (Case specific)",
    benefits: [
      "Fully customized to complement your facial features",
      "Preview of results through digital mockups before beginning",
      "Corrects multiple cosmetic flaws simultaneously",
      "Boosts self-esteem and facial harmony"
    ],
    procedure: [
      "Facial scanning, 3D imaging, and high-definition photography",
      "Digital mock-up and 3D wax-up modeling in the lab",
      "Preparation of teeth and application of trial smile",
      "Bonding of final custom-crafted ultra-thin porcelain veneers or crowns"
    ],
    faqs: [
      {
        q: "What is a trial smile?",
        a: "A trial smile is a temporary mockup placed on your teeth, allowing you to see and feel the planned results in your mouth before any permanent changes are made."
      },
      {
        q: "How long does a smile design procedure take?",
        a: "Usually, a complete smile design can be finalized in 2 to 3 visits spread over 7 to 10 days."
      }
    ]
  },
  {
    id: "cosmetic-dentist-ahmedabad",
    title: "Cosmetic Dentistry",
    icon: "Heart",
    shortDescription: "Aesthetic tooth bonding, composite fillings, veneers, and contouring for flawless teeth.",
    longDescription: "Cosmetic Dentistry focuses on enhancing the overall appearance of your teeth, gums, and bite. Whether you are dealing with minor chips, structural fractures, uneven spaces, or misshapen teeth, our cosmetic procedures offer conservative and aesthetic solutions. We utilize high-quality composite bonding resins, ceramic inlays, and ultra-thin veneers to repair teeth seamlessly and create a balanced, bright smile.",
    image: "https://images.unsplash.com/photo-1471864190281-a93a3070b6de?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹2,500 - ₹15,000 per tooth",
    benefits: [
      "Seamless blending with surrounding natural teeth",
      "Conservative treatments that preserve maximum tooth structure",
      "Stain-resistant porcelain materials",
      "Restores chipped, cracked, and worn-down teeth"
    ],
    procedure: [
      "Comprehensive aesthetic evaluation and shade selection",
      "Conservative preparation of the tooth enamel",
      "Application and sculpting of resin bonding or cementation of ceramic veneer",
      "Polishing to a high-gloss natural luster"
    ],
    faqs: [
      {
        q: "Is cosmetic dentistry covered by insurance?",
        a: "Purely cosmetic treatments are typically not covered by insurance. However, procedures that also restore tooth function, like ceramic crowns, might have partial coverage."
      },
      {
        q: "What is the difference between bonding and veneers?",
        a: "Bonding uses composite resin applied directly and cured in one visit. Veneers are custom-made thin shells crafted in a lab from durable porcelain, requiring two visits."
      }
    ]
  },
  {
    id: "braces-and-aligners-ahmedabad",
    title: "Braces & Aligners",
    icon: "Smile",
    shortDescription: "Clear aligners and metal/ceramic braces for correcting crooked teeth and bite misalignments.",
    longDescription: "Straighten your teeth comfortably with our modern orthodontic treatments. We offer traditional metal braces, discreet tooth-colored ceramic braces, and state-of-the-art Clear Aligners (invisible braces). Clear aligners are custom-made, removable plastic trays that gradually shift your teeth into the correct position. They are virtually invisible, letting you eat, brush, and speak normally while correcting crowded teeth, crossbites, underbites, and gaps.",
    image: "https://images.unsplash.com/photo-1598256989800-fe5f95da9787?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹25,000 - ₹1,50,000",
    benefits: [
      "Discreet and invisible options like Clear Aligners",
      "Improves chewing, speech, and jaw joint health",
      "Easier cleaning of straight teeth reduces cavity risks",
      "Custom digital treatment planning with visible progress"
    ],
    procedure: [
      "Digital scanning and 3D modeling of your current bite",
      "Creating a digital simulation of teeth movement and custom tray fabrication",
      "Fitting of braces or receiving your set of progressive clear aligners",
      "Regular check-ups every 4-8 weeks to monitor progress and adjust treatment"
    ],
    faqs: [
      {
        q: "Are clear aligners suitable for adults?",
        a: "Absolutely! In fact, clear aligners are highly popular among working professionals in Ahmedabad because they are virtually invisible and removable."
      },
      {
        q: "How many hours a day must I wear aligners?",
        a: "To achieve optimal results, you must wear your clear aligners for 20 to 22 hours daily, removing them only to eat, drink, brush, and floss."
      }
    ]
  },
  {
    id: "pediatric-dentist-ahmedabad",
    title: "Pediatric Dentistry",
    icon: "Baby",
    shortDescription: "Gentle, child-friendly preventive dental care, dental sealants, and cavity fillings.",
    longDescription: "Our pediatric dentistry program is designed to make dental visits fun, educational, and completely stress-free for children. We focus heavily on preventive treatments like fluoride applications and dental sealants to protect young enamel from cavities. Our specialized pediatric dentist is trained to manage children's anxieties and build positive oral hygiene habits that last a lifetime, ensuring their milk teeth and emerging permanent teeth stay healthy.",
    image: "https://images.unsplash.com/photo-1445527815219-ecbfec67492e?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹800 - ₹4,000",
    benefits: [
      "Child-friendly, comforting dental environment",
      "Prevents early childhood tooth decay (baby bottle tooth decay)",
      "Dental sealants reduce cavity risks by up to 80%",
      "Monitors jaw development and early orthodontic needs"
    ],
    procedure: [
      "Playful orientation and gentle exam of gums and teeth",
      "Mild cleaning and tooth-friendly fluoride application to strengthen enamel",
      "Application of protective sealants over deep grooves in back molars",
      "Interactive tooth brushing education for parents and children"
    ],
    faqs: [
      {
        q: "At what age should a child first visit the dentist?",
        a: "The American Academy of Pediatric Dentistry recommends a child's first visit should occur by their first birthday or within 6 months of their first milk tooth emerging."
      },
      {
        q: "What are dental sealants?",
        a: "Sealants are thin, protective plastic coatings painted onto the chewing surfaces of the back molars to prevent food and bacteria from getting trapped."
      }
    ]
  },
  {
    id: "wisdom-tooth-removal-ahmedabad",
    title: "Wisdom Tooth Removal",
    icon: "ShieldAlert",
    shortDescription: "Safe and surgical extraction of impacted, painful, or misaligned wisdom teeth.",
    longDescription: "Wisdom teeth, or third molars, are the last teeth to emerge, typically in late adolescence. Often, there is not enough space in the mouth, leading to 'impacted' wisdom teeth that grow at odd angles, damage neighboring teeth, or cause painful gum infections. Our clinic performs wisdom tooth extractions with high surgical precision using local anesthesia or conscious sedation, ensuring maximum patient comfort and quick recovery times.",
    image: "https://images.unsplash.com/photo-1598256989800-fe5f95da9787?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹3,500 - ₹9,000 per tooth",
    benefits: [
      "Relieves pain, jaw stiffness, and sinus pressure",
      "Prevents damage to adjacent healthy molars",
      "Eliminates hard-to-clean pockets that foster infection",
      "Quick healing with advanced suture materials and platelet-rich fibrin (PRF) protocols"
    ],
    procedure: [
      "Digital panoramic X-ray or 3D scan to check root morphology",
      "Administration of local anesthesia to ensure complete numbness",
      "Precise surgical extraction and cleaning of the socket site",
      "Placement of dissolvable sutures and recovery instructions"
    ],
    faqs: [
      {
        q: "How long is the recovery after wisdom tooth extraction?",
        a: "Most patients recover fully within 3 to 5 days. You will need to consume soft foods and avoid drinking through a straw to protect the healing clot."
      },
      {
        q: "Is it always necessary to remove wisdom teeth?",
        a: "No, if your wisdom teeth are fully erupted, functional, painless, aligned correctly, and easy to clean, they do not need to be removed."
      }
    ]
  },
  {
    id: "gum-treatment-ahmedabad",
    title: "Gum Treatment",
    icon: "Activity",
    shortDescription: "Deep scaling, root planing, and laser therapy to treat bleeding, swollen gums and pyorrhea.",
    longDescription: "Gum health is the foundation of a healthy mouth. Gum disease (gingivitis or periodontitis) begins when plaque buildup causes inflammation of the gums, leading to bleeding, bad breath, and eventual tooth loss. We offer periodontal therapies ranging from deep scaling and root planing to advanced laser-assisted pocket sterilization. Our targeted treatments eradicate bacteria, heal infected gum tissues, and restore support to loose teeth.",
    image: "https://images.unsplash.com/photo-1588776814546-1ffcf47267a5?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹2,000 - ₹12,000",
    benefits: [
      "Stops bleeding gums and chronic bad breath (halitosis)",
      "Prevents early tooth loss by strengthening bone support",
      "Laser gum sterilization minimizes discomfort and speeds up healing",
      "Reduces systemic inflammation linked to heart disease and diabetes"
    ],
    procedure: [
      "Measuring gum pockets with a specialized periodontal probe",
      "Deep scaling to remove sub-gingival calculus (tartar)",
      "Root planing to smooth tooth roots, making it harder for bacteria to colonize",
      "Laser sterilization of gum pockets and localized antibiotic gel application"
    ],
    faqs: [
      {
        q: "Why do my gums bleed when I brush?",
        a: "Bleeding is a classic sign of gingivitis (gum inflammation). It is caused by bacterial plaque buildup and requires a professional scaling to heal."
      },
      {
        q: "What is the difference between scaling and deep cleaning?",
        a: "Standard scaling cleans the visible tooth surfaces above the gumline. Deep cleaning (root planing) cleans below the gumline to eradicate bacteria from the roots."
      }
    ]
  },
  {
    id: "crowns-bridges-ahmedabad",
    title: "Crowns & Bridges",
    icon: "Award",
    shortDescription: "Premium Zirconia and E-max ceramic restorations to strengthen and replace teeth.",
    longDescription: "Restore damaged, weakened, or missing teeth with our premium crown and bridge restorations. A dental crown is a custom-fit cap that covers a damaged tooth to restore its shape, size, strength, and appearance. A dental bridge replaces one or more missing teeth by anchoring custom prosthetic crowns to adjacent healthy teeth. We utilize metal-free, high-strength Zirconia and CAD/CAM E-Max ceramics to ensure natural aesthetics and outstanding durability.",
    image: "https://images.unsplash.com/photo-1606811971618-4486d14f3f99?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹6,000 - ₹18,000 per unit",
    benefits: [
      "Restores natural chewing structure and appearance",
      "Metal-free Zirconia and E-Max ceramics provide unmatched aesthetics",
      "Bridges prevent adjacent teeth from shifting out of place",
      "Durable, long-lasting materials that resist stains"
    ],
    procedure: [
      "Preparation of the target teeth by gently reshaping them",
      "Taking highly accurate digital scans (intraoral scanner) or traditional impressions",
      "Fitting a temporary crown/bridge to protect the prepped teeth",
      "Fabrication in an advanced dental lab and bonding the permanent restoration"
    ],
    faqs: [
      {
        q: "Which material is better: Zirconia or Metal-Ceramic?",
        a: "Zirconia is highly recommended for its superior strength, bio-compatibility, and natural translucency, which prevents dark lines near the gumline."
      },
      {
        q: "How do I care for my dental bridge?",
        a: "You must brush twice daily, floss under the bridge using a specialized threader, and visit our Ahmedabad clinic for routine check-ups."
      }
    ]
  },
  {
    id: "dentures-ahmedabad",
    title: "Dentures",
    icon: "Grid",
    shortDescription: "Lightweight, comfortable, and stable custom dentures (BPS and implant-supported).",
    longDescription: "Dentures are removable dental appliances designed to replace missing teeth and surrounding tissues. We provide modern, high-comfort partial dentures (for a few missing teeth) and complete dentures (for replacing all teeth). Using advanced BPS (Bio-functional Prosthetic System) guidelines and flexible, lightweight materials, we ensure your dentures fit comfortably, look natural, and do not slip while speaking or eating. Implant-supported options are also available for maximum stability.",
    image: "https://images.unsplash.com/photo-1629909613654-28e377c37b09?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹12,000 - ₹45,000",
    benefits: [
      "Improves speech and restores chewing function",
      "Supports facial muscles, preventing a sunken appearance",
      "Modern flexible dentures are lightweight and highly comfortable",
      "Implant-supported options eliminate slipping or the need for adhesives"
    ],
    procedure: [
      "Comprehensive examination of gums and jawbone structure",
      "Taking detailed impressions to create custom wax mock-ups",
      "Trial fitting of the wax denture to adjust tooth alignment and color",
      "Final processing, delivery, and customized homecare coaching"
    ],
    faqs: [
      {
        q: "What are implant-supported dentures?",
        a: "These are dentures anchored securely to 2-4 dental implants placed in the jaw, preventing any slipping, clicking, or discomfort during meals."
      },
      {
        q: "Should I wear my dentures at night?",
        a: "We advise removing your dentures at night. This allows your gums and bone tissues to rest, preventing fungal infections and bone resorption."
      }
    ]
  },
  {
    id: "tooth-extraction-ahmedabad",
    title: "Tooth Extraction",
    icon: "ShieldAlert",
    shortDescription: "Painless, conservative extraction of severely fractured or non-restorable teeth.",
    longDescription: "While our priority is always to save your natural teeth, there are situations where a tooth is too severely damaged, infected, or fractured to be repaired. In these cases, a gentle extraction is the best way to safeguard your overall oral health. We perform extractions with a focus on preserving the surrounding bone structure, which is crucial if you plan to replace the tooth with a dental implant in the future.",
    image: "https://images.unsplash.com/photo-1598256989800-fe5f95da9787?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹1,500 - ₹4,500",
    benefits: [
      "Provides quick relief from chronic pain and severe infection",
      "Prevents bacterial infections from spreading to adjacent teeth",
      "Preserves surrounding bone structure for future implants",
      "Conscious sedation options available for anxious patients"
    ],
    procedure: [
      "Taking localized digital X-rays to assess root alignment",
      "Administering local anesthetic to completely numb the area",
      "Gentle lifting and removal of the non-restorable tooth",
      "Placement of a sterile gauze pad and providing step-by-step aftercare instructions"
    ],
    faqs: [
      {
        q: "What should I eat after an extraction?",
        a: "You should stick to cold, soft foods like yogurt, ice cream, smoothies, and room-temperature soup for the first 24 to 48 hours."
      },
      {
        q: "How soon can I replace the extracted tooth?",
        a: "We can place a temporary restoration immediately, or begin planning a permanent dental implant or bridge once the extraction socket heals (usually 2-3 months)."
      }
    ]
  },
  {
    id: "preventive-dental-care-ahmedabad",
    title: "Preventive Dental Care",
    icon: "Shield",
    shortDescription: "Fluoride treatments, cavity-preventing sealants, nightguards, and regular diagnostic scans.",
    longDescription: "Preventive dentistry is the key to maintaining a healthy smile for a lifetime. By focus on stopping dental issues before they start, we save you from future discomfort and costly dental procedures. Our preventive care program includes regular professional cleanings, fluoride applications to strengthen enamel, dental sealants to protect chewing surfaces, custom nightguards for grinding (bruxism), and high-resolution oral cancer screenings.",
    image: "https://images.unsplash.com/photo-1471864190281-a93a3070b6de?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹1,000 - ₹5,000",
    benefits: [
      "Drastically reduces the risk of developing cavities and gum disease",
      "Detects early warning signs of teeth grinding and TMJ problems",
      "Highly affordable treatments that prevent expensive procedures later",
      "Keeps teeth clean, breath fresh, and smile bright"
    ],
    procedure: [
      "Detailed visual assessment and low-radiation digital scans",
      "Gentle plaque removal and tooth surface polishing",
      "Application of professional-grade fluoride gel or varnish",
      "Custom scan and impressions for grinding guards or athletic mouthguards if needed"
    ],
    faqs: [
      {
        q: "Do adults need fluoride treatment?",
        a: "Yes, fluoride treatments are highly beneficial for adults with sensitive teeth, gum recession, dry mouth, or a history of frequent cavities."
      },
      {
        q: "What is bruxism, and how does a nightguard help?",
        a: "Bruxism is the involuntary grinding or clenching of teeth, usually during sleep. A custom-fit nightguard acts as a protective barrier, preventing enamel wear and relieving jaw pain."
      }
    ]
  },
  {
    id: "emergency-dentist-ahmedabad",
    title: "Emergency Dental Treatment",
    icon: "ShieldAlert",
    shortDescription: "Immediate care for knocked-out teeth, severe toothaches, jaw fractures, and dental abscesses.",
    longDescription: "A dental emergency can be frightening and painful. Whether you are dealing with a severe, throbbing toothache, a knocked-out tooth from sports, a cracked crown, or a swelling in your gums, immediate medical attention is essential. Our Ahmedabad clinic is fully prepared to handle urgent dental situations, offering same-day appointments, pain-relief treatments, root canals, and trauma management to resolve pain and protect your smile.",
    image: "https://images.unsplash.com/photo-1588776814546-1ffcf47267a5?w=800&auto=format&fit=crop&q=80",
    priceRange: "₹1,500 - ₹8,000",
    benefits: [
      "Priority same-day appointments to relieve intense pain",
      "Expert trauma management for knocked-out or fractured teeth",
      "Preserves teeth that might otherwise require extraction",
      "Equipped with advanced diagnostic and emergency therapeutic tools"
    ],
    procedure: [
      "Immediate triage and diagnostic X-rays to locate the issue",
      "Local anesthesia or pain relief treatment administered immediately",
      "Urgent repair, emergency root canal, tooth re-implantation, or abscess drainage",
      "Prescribing anti-inflammatory medications and antibiotics for safe recovery"
    ],
    faqs: [
      {
        q: "What should I do if a permanent tooth is knocked out?",
        a: "Pick it up by the crown (do not touch the root), gently rinse it, and try to place it back in its socket. If that is not possible, keep it in a cup of cold milk and visit us within 1 hour."
      },
      {
        q: "Do you offer emergency dental care on Sundays or holidays?",
        a: "Yes. We maintain an active on-call doctor for urgent emergency dental support in Ahmedabad. Call us immediately at +91 7600 583 156."
      }
    ]
  }
];
